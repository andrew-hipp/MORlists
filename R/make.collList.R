...
make.collList <- function(x,
  ...
  if(useCollSheets) {
	...
	out.plant <- data.frame(
	              ...
	              grid = paste(dat.lc$gardensubarea5, dat.lc$gardensubarea6, sep = '/'),
	              bgLoc = substr(gsub(":::|::", ":",
	                             paste(dat.lc$gardenlocalityname,
  	                              ifelse(dat.lc$gardensubarea1 != '', dat.lc$gardensubarea1, ''),
  	                              ifelse(dat.lc$gardensubarea2 != '', dat.lc$gardensubarea2, ''),
  	                              sep = ":") # close paste
	                               ), 1, trimL),
